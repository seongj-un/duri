...
- **금액은 전부 `BIGINT`(원 단위 정수).** 부동소수점을 쓰지 않는다. 상대 몫을 먼저 내림해서 구하므로, 부담 비율을 곱해 남는 1원은 결제자가 흡수한다.
...
`web/` 에 Vite + React + TypeScript SPA 가 있다. 화면 정의·컬러 토큰·타이포는
노션 문서 *PairPay 프론트 디자인* 이 원본이고, 구현 결정은 `docs/superpowers/specs/2026-09-07-frontend-design.md` 에 있다.

```bash
cd web && npm install && npm run dev
```

**포트 3000 은 고정이다.** 백엔드가 CORS 허용 오리진·OAuth 착지 주소·초대 링크를 모두
...
- **액세스 토큰은 메모리에만 둔다.** `localStorage` 에 두면 XSS 한 번에 새어 나가고,
  리프레시 쿠키가 HttpOnly 라 새로고침 후 `POST /auth/token` 으로 되찾을 수 있어 저장할 이유가 없다.
- **재발급은 한 번에 하나만.** 여러 요청이 동시에 401 을 받아도 진행 중인 Promise 를 나눠 쓴다.
...
- **SSE 는 fetch 기반**(`@microsoft/fetch-event-source`)이다. 기본 `EventSource` 는 헤더를 못 붙인다.
  이벤트의 `period` 로 그 달의 쿼리만 무효화하고, 내가 일으킨 이벤트(`actorId`)는 무시한다.

화면은 9개다. 노션이 정의한 6개(로그인 · 파트너 연결 · 홈 · 지출 등록 · 월별 내역 · 정산)에
흐름상 필요한 3개(OAuth 착지 · space 만들기 · 초대 수락)를 더했고, 정산 화면의 송금 안내가
비지 않도록 계좌 등록까지 붙였다. 반복지출·부담비율 프리셋·알림함·지출 추이·PWA 는 아직 없다.
...
3개월 로드맵 기준 **W1–W12 완료 (백엔드)**, 프론트엔드는 핵심 화면까지.
...
- [x] 프론트엔드 핵심 화면 (로그인 · 페어링 · 홈 · 지출 등록 · 월별 내역 · 정산)
- [ ] 프론트엔드 나머지 (반복지출 · 부담비율 프리셋 · 알림함 · 지출 추이)
- [ ] PWA
