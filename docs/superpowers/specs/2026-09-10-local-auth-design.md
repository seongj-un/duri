...
테스트가 아니라 앱을 띄워 브라우저로 전 흐름을 밟았다.
포트 8080 을 다른 프로세스가 쓰고 있어 백엔드는 8081 로,
프론트는 `VITE_BACKEND_ORIGIN=http://localhost:8081` 로 띄웠다.

| 단계 | 결과 |
|---|---|
| 회원가입(성준) | 즉시 로그인 상태가 되어 온보딩으로 이동 |
| 커플 생성 | "우리집 가계부" → `/link` |
| 초대 링크 발급 | 화면 진입 시 자동 발급, 만료 7일 후로 표시 |
| 비로그인 미리보기 | 로그아웃 뒤 링크를 열어 초대자 닉네임과 space 이름만 노출되는 것을 확인 |
| pendingInvite 복귀 | 가입 후 초대 페이지로 자동 복귀, 버튼이 "수락하고 시작하기" 로 전환 |
| 수락(지현) | 홈 진입, 헤더에 space 이름 |

DB 상태도 확인했다. `password_hash` 는 두 계정 모두 `$2a$10$` 로 시작하는 60자,
커플은 ACTIVE 에 멤버 2명, 초대는 accepted 이고 revoked 가 아니다.
브라우저 콘솔 오류는 없었다.

레이트 리밋은 같은 이메일로 로그인을 반복해 확인했다.
...
10회째에 걸린 것이 설계대로다. 성준의 회원가입 1회가 이미 그 이메일 카운터에
잡혀 있어 합계가 11이 된 것이고, 가입도 같은 카운터로 센다는 것이 함께 확인됐다.

### 확인하지 못한 것

`/link` 의 "상대가 들어오면 자동으로 넘어가요" — SSE 실시간 전환.
한 브라우저에서 로그아웃하며 진행해 두 세션을 동시에 띄우지 못했다.
확인하려면 시크릿 창이 하나 더 필요하다.
