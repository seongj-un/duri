 ...
 *
 * 부담 몫은 DB 에서 계산한다. Expense.partnerShare 와 같은 식(정수 나눗셈, 내림)을 쓰므로
...
class ExpenseQueryRepository(
    ...
    /** amount * (100 - rate) / 100 (정수 나눗셈). 엔티티와 같이 상대 몫을 먼저 내림한다. */
    ...
    /** amount - partnerShare. 나누어떨어지지 않고 남는 1원은 결제자가 흡수한다. */
